...
def write_10X_h5(filename, matrix, features, barcodes, genome = 'GRCh38', datatype = 'Peak'):
    ...
    mat.create_dataset('barcodes', data=B, compression='gzip', fletcher32=True)
    mat.create_dataset('data', data=M.data, compression='gzip', fletcher32=True)
    mat.create_dataset('indices', data=M.indices, compression='gzip', fletcher32=True)
    mat.create_dataset('indptr', data=M.indptr, compression='gzip', fletcher32=True)
    mat.create_dataset('shape', data=M.shape)
    fet = mat.create_group('features')
    fet.create_dataset('_all_tag_keys', data=AT)
    fet.create_dataset('feature_type', data=FT, compression='gzip', fletcher32=True)
    fet.create_dataset('genome', data=GM, compression='gzip', fletcher32=True)
    fet.create_dataset('id', data=P, compression='gzip', fletcher32=True)
    fet.create_dataset('name', data=P, compression='gzip', fletcher32=True)
    f.close()
